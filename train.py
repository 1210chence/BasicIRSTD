...
import numpy as np
import os
from model.MTUNet.load_param_data import  load_dataset, load_param
from model.MTUNet.parse_args_train import  parse_args
from model.MTUNet.MTU_Net import  res_UNet, Res_block
from model.MTUNet.loss import *
os.environ['KMP_DUPLICATE_LIB_OK'] = 'TRUE'

parser = argparse.ArgumentParser(description="PyTorch BasicIRSTD train")
parser.add_argument("--model_names", default=['UIUNet'], type=list, 
                    help="model_name: 'ACM', 'ALCNet', 'DNANet', 'ISNet', 'UIUNet', 'RDIAN', 'ISTDU-Net', 'U-Net', 'RISTDnet','MTUNet'")              
...
def train():
  # train_set = TrainSetLoader(dataset_dir=opt.dataset_dir, dataset_name=opt.dataset_name, patch_size=opt.patchSize, img_norm_cfg=opt.img_norm_cfg)
    ...
    # nb_filter, num_blocks = load_param(channel_size =args.channel_size ,backbone =args.backbone)
    ...
    # model = model.cuda()
    # net= model.apply(weights_init_xavier)
    ...
    for idx_epoch in range(epoch_state, opt.nEpochs):
        for idx_iter, (img, gt_mask) in enumerate(train_loader):
            ...
            loss_map,loss = FocalIoULoss(pred, gt_mask)
...
def test(save_pth):
    ...
    nb_filter, num_blocks = load_param(channel_size =args.channel_size ,backbone =args.backbone)
    net = res_UNet(num_classes=1, input_channels=args.in_channels, block=Res_block, num_blocks=num_blocks,nb_filter=nb_filter)# 实例化res_UNet模型
