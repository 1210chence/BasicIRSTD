...
#     # Predefined normalization configs for specific datasets
#     if dataset_name == 'NUAA-SIRST':
#         return dict(mean=101.06385040283203, std=34.619606018066406)
#     elif dataset_name == 'NUDT-SIRST':
#         return dict(mean=107.80905151367188, std=33.02274703979492)
#     elif dataset_name == 'IRSTD-1K':
#         return dict(mean=87.4661865234375, std=39.71953201293945)
#     elif dataset_name == 'NUDT-SIRST-Sea':
#         return dict(mean=43.62403869628906, std=18.91838264465332)
#     elif dataset_name == 'SIRST4':
#         return dict(mean=62.10432052612305, std=23.96998405456543)
#     elif dataset_name == 'IRDST-real':
#         return {'mean': 101.54053497314453, 'std': 56.49856185913086}
#     else:
#         # Attempt to calculate normalization stats from images in the dataset
...
#                 img_path = (img_dir + img_pth).replace('//', '/') + ext
#                 try:
#                     print(f"Trying to open: {img_path}")
#                     img = Image.open(img_path).convert('I')
#                     break  # Image found, no need to try other extensions
#                 except FileNotFoundError:
#                     print(f"File not found: {img_path}")
#                     continue  # Try next extension
            
#             # If we're here, none of the image extensions were found
#             print(f"No valid image found for path: {img_pth}")
#             continue  # Skip to next image in list
            
#             # Convert image to numpy array and compute statistics
#             img_arr = np.array(img, dtype=np.float32)
#             mean_list.append(img_arr.mean())
#             std_list.append(img_arr.std())
        
#         # Calculate and return the mean and std if any images were processed
#         if mean_list and std_list:
#             return dict(mean=float(np.array(mean_list).mean()), std=float(np.array(std_list).mean()))
#         else:
#             print("No valid images were found to compute normalization configuration.")
#             return None  # Or provide a default normalization config
def get_img_norm_cfg(dataset_name, dataset_dir):
    if  dataset_name == 'NUAA-SIRST':
        ...
        for img_pth in img_list:
            ...
            # print(flag)
            try:
                # print((img_dir + img_pth).replace('//', '/') + '.png')
                # sys.exit()
